...
import { 
  ...
  Database, Eye, Mail, UserPlus, Pencil, Trash, Wand2, Save
} from 'lucide-react';
...
const TRANSLATIONS = {
  fr: {
    ...
    legal_2_title: "2. Traitement des données et Sécurité", legal_2_text: "Nous traitons vos données conformément au RGPD et à la LPRPDE. Les données (formulaires, API) sont utilisées strictement pour fournir le service. Vos informations sont stockées de manière cryptée. Nous ne vendons pas vos données.",
    ...
    analysis_text_3: "Relancez les", analysis_text_4: "prospects en attente d'ici 48 heures.",
    ...
    btn_save_msg: "Enregistrer le message"
  },
  en: {
    ...
    legal_2_title: "2. Data Processing and Security", legal_2_text: "We process data in accordance with GDPR and PIPEDA. Data is used strictly to provide the service and is stored securely. We do not sell your data.",
    ...
    analysis_text_3: "Follow up with the", analysis_text_4: "pending prospects within 48 hours.",
...
export default function NterPlatform() {
  ...
  const [clientFormData, setClientFormData] = useState({ id: null, name: '', target: '', agendaUrl: '', crm: '', knowledge_base: '' });
  ...
  const handleSaveClient = async (e: React.FormEvent) => {
    ...
    if (clientFormData.id) {
        const { data, error } = await supabase.from('clients').update({
            name: clientFormData.name, target: clientFormData.target, agendaurl: clientFormData.agendaUrl, crm: clientFormData.crm, knowledge_base: clientFormData.knowledge_base
    ...
    setClientFormData({ id: null, name: '', target: '', agendaUrl: '', crm: '', knowledge_base: '' }); setIsSaving(false);
  };
      ...
      if (aiData.newLead) {
        ...
        const { data: dbData, error } = await supabase.from('prospects').insert([
          { 
            ...
            name: leadName, 
            email: aiData.newLead.email || "", 
            phone: aiData.newLead.phone || "", 
            address: aiData.newLead.address || "", 
            firstcontact: today, 
            status: 'pending', 
            followup: followUpStr, 
            email_subject: aiData.newLead.email_subject, 
            email_body: aiData.newLead.email_body 
          }
        ]).select('*'); 
  ...
  const handleUpdateEmail = async () => {
    ...
    const { data, error } = await supabase.from('prospects')
      .update({ email_subject: selectedEmail.subject, email_body: selectedEmail.body })
      .eq('id', selectedEmail.id).select('*');
  ...
  const handleSendEmail = async (prospect: any) => {
    ...
    try {
      const response = await fetch('/api/send-email', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ to: prospect.email, subject: prospect.email_subject, text: prospect.email_body, prospectName: prospect.name, clientName: activeClient.name })
      });
  ...
  const handleProcessReportAction = () => { setShowReportConfigModal(false); alert(reportMode === 'auto' ? t.alert_config_saved : "Rapport affiché"); if (reportMode !== 'auto') setShowGeneratedReport(true); };
  const downloadCSV = () => { /* Logique CSV inchangée */ };
              ...
              <>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="bg-slate-800 border border-slate-700 rounded-xl p-6 flex flex-col justify-center items-center text-center space-y-4">
                    ...
                    <button onClick={() => setShowReportConfigModal(true)} className="w-full max-w-xs bg-slate-700 hover:bg-slate-600 text-white px-6 py-3 rounded-lg font-medium flex justify-center items-center gap-2 border border-slate-600 transition-colors"><FileBarChart size={18} className="text-emerald-400" /> {t.btn_report}</button>
                  </div>
                </div>
      ...
      {showGeneratedReport && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex justify-center items-center p-4 animate-in fade-in duration-200">
          <div className="bg-slate-800 border border-slate-600 rounded-xl w-full max-w-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
            <div className="flex justify-between items-center p-6 border-b border-slate-700 bg-gradient-to-r from-slate-800 to-indigo-900/20"><div><h3 className="text-xl font-bold text-white flex items-center gap-2"><FileBarChart className="text-indigo-400" /> {t.generated_title}</h3><p className="text-slate-400 text-sm mt-1">{t.client_label} {activeClient?.name}</p></div><button onClick={() => setShowGeneratedReport(false)} className="text-slate-400 hover:text-white bg-slate-900/50 p-2 rounded-full"><X size={20} /></button></div>
            <div className="p-6 overflow-y-auto space-y-6"><div className="grid grid-cols-2 md:grid-cols-4 gap-4"><div className="bg-slate-900 border border-slate-700 p-4 rounded-lg text-center"><p className="text-xs text-slate-400 uppercase font-bold mb-1">{t.kpi_total}</p><p className="text-3xl font-bold text-white">{reportStats.total}</p></div><div className="bg-emerald-900/20 border border-emerald-700/50 p-4 rounded-lg text-center"><p className="text-xs text-emerald-400 uppercase font-bold mb-1">{t.kpi_accepted}</p><p className="text-3xl font-bold text-emerald-400">{reportStats.accepted}</p></div><div className="bg-yellow-900/20 border border-yellow-700/50 p-4 rounded-lg text-center"><p className="text-xs text-yellow-400 uppercase font-bold mb-1">{t.kpi_pending}</p><p className="text-3xl font-bold text-yellow-400">{reportStats.pending}</p></div><div className="bg-red-900/20 border border-red-700/50 p-4 rounded-lg text-center"><p className="text-xs text-red-400 uppercase font-bold mb-1">{t.kpi_refused}</p><p className="text-3xl font-bold text-red-400">{reportStats.refused}</p></div></div><div className="bg-indigo-900/20 border border-indigo-500/20 p-4 rounded-lg"><h4 className="font-semibold text-indigo-300 mb-2">{t.ai_analysis}</h4><p className="text-sm text-slate-300">{t.analysis_text_1} <strong>{activeClient?.name}</strong> {t.analysis_text_2} <strong className="text-white ml-1">{reportStats.total > 0 ? Math.round((reportStats.accepted / reportStats.total) * 100) : 0}%</strong>. {t.analysis_text_3} {reportStats.pending} {t.analysis_text_4}</p></div></div>
            <div className="p-5 border-t border-slate-700 flex flex-col sm:flex-row justify-end gap-3 bg-slate-800/50"><button onClick={downloadCSV} className="px-4 py-2 text-sm font-medium bg-slate-700 hover:bg-slate-600 text-white rounded-lg flex items-center justify-center gap-2 transition-colors border border-slate-600"><Download size={16} /> {t.btn_download_csv}</button><button className="px-4 py-2 text-sm font-bold bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg shadow-md flex items-center justify-center gap-2 transition-colors"><Send size={16} /> {t.btn_send_email}</button></div>
          </div>
        </div>
      )}
    </div>
